...
for i = 1:nx
        % Step 1
        RHS(i, 2) = RHS(i, 2) - penta1(2, 8) * RHS(i, 1);
        for j = 3:ny
            RHS(i, j) = RHS(i, j) - penta1(j, 9) * RHS(i, j-2) - penta1(j, 8) * RHS(i, j-1);
        end

        % Step 2
        RHS(i, ny) = RHS(i, ny) * penta1(ny, 7);

        RHS(i, ny-1) = RHS(i, ny-1) * penta1(ny-1, 7) - penta1(ny-1, 10) * RHS(i, ny);
        for j = ny-2:-1:1
            RHS(i, j) = RHS(i, j) * penta1(j, 7) - RHS(i, j+2) * penta1(j, 5) * penta1(j, 7) - RHS(i, j+1) * penta1(j, 10);
        end
end
