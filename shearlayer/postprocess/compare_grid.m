...
Ly=Ly_mat(i);dy = Ly/ny(j); y = linspace(-Ly/2,Ly/2,ny(j))'; 
...
screen2jpeg(strcat(outputdir1,'subplot_',sprintf('%1.1f',Mc(i)),'_.png'));
end
...
Mc=[2.0];
du=2*Mc*sqrt(1.4);
%t_i=[460,460,320];t_f=[900,900,760];  %Mc=0.2
t_i=[110,110,110];t_f=[250,250,250];  %Mc=2.0
caseid="mgm_sta_c1";
for i=1%:size(cases,2)
%fname = strcat(inputdir,cases(i),'/run',mc,'/',caseid,'/postprocess/','delta_',sprintf('%1.1f',Mc),'_',caseid,'.dat');
fname = strcat(inputdir,cases(i),'/',caseid,'/postprocess/','delta_',sprintf('%1.1f',Mc),'_',caseid,'.dat');
f=readmatrix(fname);
...
set(findall(gcf,'-property','FontWeight'),'FontWeight','bold')
grid on;hAx=gca; set(hAx,'xminorgrid','off','yminorgrid','off')
screen2jpeg(strcat(outputdir1,'momentum_thick1_',caseid,'.png'));
