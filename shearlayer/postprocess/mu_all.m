...
t_i = 110; t_f = 250; time_step = 5;
time_total = t_i:time_step:t_f
mustar_phy_avg = zeros(size(time_total,2),ny);
musgs_phy_avg = zeros(size(time_total,2),ny);

diss_phy = zeros(size(time_total,2),ny);
diss_star= zeros(size(time_total,2),ny);
diss_sgs = zeros(size(time_total,2),ny);
...
diss_phy(k,:)=avgxz(func_tau,nx,nz);
diss_phy_int(k,1) = (sum(diss_phy(k,:))*dy)/(du^3);
...
diss_star(k,:)=avgxz(func_tau,nx,nz);
diss_star_int(k,1) = (sum(diss_star(k,:))*dy)/(du^3);
...
diss_sgs(k,:)=avgxz(func_tau,nx,nz);
diss_sgs_int(k,1) = (sum(diss_sgs(k,:))*dy)/(du^3);


%mustar_muphy = mustar./muphy ;  musgs_muphy = musgs./muphy ;
%mustar_phy_avg(k,:) = avgxz(mustar_muphy,nx,nz);
%musgs_phy_avg(k,:)  = avgxz(musgs_muphy,nx,nz);

k=k+1;

end

%mustar_tavg = mean(mustar_avg) ; mustar_one = mean(mustar_tavg);
%muphy_tavg = mean(muphy_avg)   ; muphy_one = mean(muphy_tavg);
%mustar_phy_tavg = mean(mustar_phy_avg)   ; musgs_phy_tavg = mean(musgs_phy_avg);

diss_phy_tavg = mean(diss_phy); diss_star_tavg = mean(diss_star); diss_sgs_tavg = mean(diss_sgs);
diss_phy_int_tavg = mean(diss_phy_int) ; diss_star_int_tavg = mean(diss_star_int); diss_sgs_int_tavg = mean(diss_sgs_int);

fname = strcat('/home/vsj/Codes/diss/diss_all_',caseid,'_',sprintf('%1.1f',Mc),'.dat');
fileID = fopen(fname,'w');
for i=1:ny
fprintf(fileID,'%1.1f %2.8f %2.8f %2.8f %2.8f\r\n',Mc,y(i),diss_phy_tavg(i),diss_star_tavg(i),diss_sgs_tavg(i));
end
fclose(fileID);

%mustar_phy_centre= mustar_phy_tavg(ny/2) ; musgs_phy_centre= musgs_phy_tavg(ny/2) ;
%mustar_phy_one = mean(mustar_phy_tavg); musgs_phy_one   = mean(musgs_phy_tavg);
%mustar_phy_int = sum(mustar_phy_tavg)*dy; musgs_phy_int = sum(musgs_phy_tavg)*dy;

fname = strcat('/home/vsj/Codes/diss/diss_all_',caseid,'.dat');
fileID = fopen(fname,'a');
fprintf(fileID,'%1.1f %2.8f %2.8f %2.8f\r\n',Mc,diss_phy_int_tavg,diss_star_int_tavg,diss_sgs_int_tavg);
fclose(fileID);

figure(1),clf
plot(y,diss_phy_tavg,'r','LineWidth',2,'DisplayName','\epsilon_{phy}');hold on
plot(y,diss_star_tavg,'b','LineWidth',2,'DisplayName','\epsilon_{*}');hold on
plot(y,diss_sgs_tavg,'g','LineWidth',2,'DisplayName','\epsilon_{sgs}');hold on
legend show;legend('Location','northeast','box','off','FontSize',10)

fnamepng = strcat('/home/vsj/Codes/diss/diss_all_',caseid,'_',sprintf('%1.1f',Mc),'.png');
