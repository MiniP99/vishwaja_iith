...
for j = 1:ny
    for k = 1:nz
        % Step 1
        RHS(2, j, k) = RHS(2, j, k) - penta1(2, 8) * RHS(1, j, k);
        for i = 3:nx
            RHS(i, j, k) = RHS(i, j, k) - penta1(i, 9) * RHS(i-2, j, k) - penta1(i, 8) * RHS(i-1, j, k);
        end

        % Step 2
        RHS(nx, j, k) = RHS(nx, j, k) * penta1(nx, 7);

        RHS(nx-1, j, k) = RHS(nx-1, j, k) * penta1(nx-1, 7) - penta1(nx-1, 10) * RHS(nx, j, k);
        for i = nx-2:-1:1
            RHS(i, j, k) = RHS(i, j, k) * penta1(i, 7) - RHS(i+2, j, k) * penta1(i, 5) * penta1(i, 7) - RHS(i+1, j, k) * penta1(i, 10);
        end
    end
end
